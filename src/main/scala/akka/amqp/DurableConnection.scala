...
class DurableConnection(private[amqp] val connectionProperties: ConnectionProperties) {
	...
	private[amqp] val durableConnectionActor = connectionProperties.system.actorOf(Props(new DurableConnectionActor(connectionProperties)), "connection" + System.currentTimeMillis)
